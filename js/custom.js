$("select").html(`
    ...
    `)
var id;$('[data-target][data-toggle=modal]').on('click',function(){id=$(this).attr('data-target');Title={'Homemeeting':"Put your interests in action, get all the details right now.",'DownloadBrochure':"Mention your whatsapp number to receive Brochure on whatsapp",'PriceSection':"Get explicit details about the pricing and on spot offers",'FloorPlan':"You are trying to download a big file, get it on Whatsapp",'Location':"Due to huge rush at site we would request you to book your sitevisit in advance",'Gallery':"Inside features of the project at a glance",'video_call':'Virtual Zoom Meetings - With the Experts','default':"PLEASE SHARE YOUR DETAILS. OUR SALES EXPERT WILL CALL YOU SHORTLY"}
$('.modal.form h4').text(Title[id])
$('.modal.form').attr('id',id).modal('show');});var projectId;var RegNo;var city;let apiDataGet=async()=>{let data=getProjectData(180).then((data)=>{phone_no=data.result.phone;whatsapp_url=data.result.wp_links_sms;whatsapp_url=whatsapp_url.split("=");whatsapp_url[2]="I want to know more about "+whatsapp_url[2];whatsapp_url=whatsapp_url.join("=");projectId=data.result.Project.p_id;city=data.result.Project.Region.city;RegNo=data.result.Project.Region.region_name;$(".phone_url").attr("href","tel:"+phone_no+"");$(".whatsapp_url").attr("href",whatsapp_url+".");$(".phone_no").html(phone_no);}).catch((error)=>{var whatsapp_url="https://api.whatsapp.com/send?phone=917304412403&text=Hi!"
$(".whatsapp_url").attr("href",whatsapp_url);$(".phone_url").attr("href","tel:"+917304927701+"");$(".phone_no").html("917304927701")});}
apiDataGet();var param_nationality=1;$('#dropEnquiry').on("submit",function(){debugger
var name=$('#dropEnquiry_Name').val();var email=null;var number=$('#dropEnquiry_Number').val();var country_code=$('#dropEnquiry_CountryCode').val();saveLead(name,email,number,country_code,'top_enquire_form');return!1})
$('#ModalForm').on("submit",function(){debugger
var name=$('#ModalForm_Name').val();var email=null;var number=$('#ModalForm_Number').val();var country_code=$('#ModalForm_CountryCode').val();saveLead(name,email,number,country_code,id+'_overview_homepage');return!1})
$('#PopUpForm').on("submit",function(){debugger;var name=$('#PopUp_name').val();var email="Noe email";var number=$('#PopUp_number').val();var country_code=$('#PopUp_CountryCode').val();saveLead(name,email,number,country_code,'PopUp_form_overview_homepage');return!1})
$('#priceForm').on("submit",function(){debugger;var name=$('#pric_ename').val();var email="Noe email";var number=$('#price_number').val();var country_code=$('#price_CountryCode').val();saveLead(name,email,number,country_code,'price_form_overview_homepage');return!1})
$('#price1Form').on("submit",function(){debugger;var name=$('#price1_name').val();var email="Noe email";var number=$('#price1_number').val();var country_code=$('#price1_CountryCode').val();saveLead(name,email,number,country_code,'price1_form_overview_homepage');return!1})
$('#price2Form').on("submit",function(){debugger;var name=$('#price2_name').val();var email="Noe email";var number=$('#price2_number').val();var country_code=$('#price2_CountryCode').val();saveLead(name,email,number,country_code,'price2_form_overview_homepage');return!1})
$('#price3Form').on("submit",function(){debugger;var name=$('#price3_name').val();var email="Noe email";var number=$('#price3_number').val();var country_code=$('#price3_CountryCode').val();saveLead(name,email,number,country_code,'price3_form_overview_homepage');return!1})
$('#price4Form').on("submit",function(){debugger;var name=$('#price4_name').val();var email="Noe email";var number=$('#price4_number').val();var country_code=$('#price4_CountryCode').val();saveLead(name,email,number,country_code,'price4_form_overview_homepage');return!1})
$('#price5Form').on("submit",function(){debugger;var name=$('#price5_name').val();var email="Noe email";var number=$('#price5_number').val();var country_code=$('#price5_CountryCode').val();saveLead(name,email,number,country_code,'price5_form_overview_homepage');return!1})
$('#price6Form').on("submit",function(){debugger;var name=$('#price6_name').val();var email="Noe email";var number=$('#price6_number').val();var country_code=$('#price6_CountryCode').val();saveLead(name,email,number,country_code,'price6_form_overview_homepage');return!1})
$('#price7Form').on("submit",function(){debugger;var name=$('#price7_name').val();var email="Noe email";var number=$('#price7_number').val();var country_code=$('#price7_CountryCode').val();saveLead(name,email,number,country_code,'price7_form_overview_homepage');return!1})
async function saveLead(name,email,number,country_code,pref){debugger;var getUtmData=queryForm();var ipAddress=await getIpAddress();var user_device=deviceData();var user_browser=browserData();var obj={};obj.name=name;obj.number=number;obj.email=email;obj.country_code=country_code;obj.nationality=param_nationality;obj.source_id=31;obj.project_id=projectId;if(getUtmData){obj.Utm={utm_medium:getUtmData.utmmedium,utm_source:getUtmData.utmsource,utm_content:getUtmData.utmcontent,utm_term:getUtmData.utmterm};}
obj.Digital={user_device:user_device,user_browser:user_browser,campaing_type:getUtmData?getUtmData.utmcampaign:null,launch_name:"",client_ipaddress:ipAddress,client_pref:pref}
if(pref=='DownloadBrochure_overview_homepage'){SendLead(obj,"ThankYou/thankyou.html");}else{SendLead(obj,"ThankYou/thankyou.html");}}